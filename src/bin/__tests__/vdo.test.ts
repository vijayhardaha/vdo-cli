...
vi.mock('../../commands/download.js', () => ({ setupDownload: vi.fn() }));
vi.mock('../../commands/convert.js', () => ({ setupConvert: vi.fn() }));
vi.mock('../../commands/compress.js', () => ({ setupCompress: vi.fn() }));
vi.mock('../../commands/speedup.js', () => ({ setupSpeedup: vi.fn() }));
vi.mock('../../commands/audio.js', () => ({ setupAudio: vi.fn() }));
...
describe('bin/vdo', () => {
  ...
  it('should register all 5 commands on the program', async () => {
    const { setupDownload } = await import('../../commands/download.js');
    const { setupConvert } = await import('../../commands/convert.js');
    const { setupCompress } = await import('../../commands/compress.js');
    const { setupSpeedup } = await import('../../commands/speedup.js');
    const { setupAudio } = await import('../../commands/audio.js');
